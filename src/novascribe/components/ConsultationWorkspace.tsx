...
export default function ConsultationWorkspace({ consultation, patientHistory, onFinish, onSaveReport, onExit, onNewSession, onSelectSession, onSessionUpdate }: ConsultationWorkspaceProps) {
  ...
  // Finalize a stopped recording: show the finalized transcript, translate it if
  // a specific output language is selected, then auto-generate the report from
  // the COMPLETE transcript. Runs from onend so the recognizer's last flushed
  // final result is already committed (nothing spoken is lost).
  const finalizeStop = async () => {
    ...
    // is what makes externally-played audio transcribe correctly. Auto language
    // detection (no forced language); the backend does not translate or summarise.
    // Falls back to the live transcript if nothing usable was recorded or the
    // backend errors, so direct-voice recording keeps working regardless.
    try {
    ...
    // Source-of-truth transcript (raw spoken language) + what is shown.
    setOriginalTranscript(finalText);
    setDisplayedTranscript(finalText);

    if (finalText && language && language !== 'auto') {
      setIsTranslating(true);
      try {
        const translated = await translateTranscript(finalText, language);
    ...
    // Auto-generate the report from the full updated transcript.
    if (finalText) await runReportGeneration(finalText);
  };
  ...
  const stopWhisperRecording = () => {
    ...
    recorder.onstop = async () => {
      ...
      try {
        // Whisper returns the raw spoken-language text from THIS recording only
        // (auto-detected source). No language is forced, so mixed Hindi / English
        // / Hinglish / Urdu speech is detected automatically. Output-language
        // translation is handled separately, on demand.
        ...
        if (text) {
          // Append raw Whisper output to the original transcript (source of truth).
          const newOriginal = (originalTranscript ? `${originalTranscript} ${text}` : text).trim();
          setOriginalTranscript(newOriginal);

          // Show it converted to the currently selected output language.
          if (!language || language === 'auto') {
            ...
            try {
              ...
              setError(d || 'Failed to translate the transcript.');
              // Fall back to the original so the transcript is never lost.
              setDisplayedTranscript(newOriginal);
            } finally {
              setIsTranslating(false);
            }
  ...
  const handleUploadChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Append to the transcript (same source-of-truth model as recording), then
      // show it in the currently selected output language.
      const newOriginal = (originalTranscript ? `${originalTranscript} ${text}` : text).trim();
      setOriginalTranscript(newOriginal);
  ...
  // Changing the dropdown re-converts the ENTIRE transcript into the new language.
  const handleLanguageChange = async (newLang: string) => {
    setLanguage(newLang);

    // Translate from the raw spoken text when we have it; otherwise fall back to
    // whatever is currently shown (e.g. text typed/edited by hand). This makes
    // language selection convert the COMPLETE transcript, not just recorded text.
    const source = (originalTranscript.trim() || displayedTranscript.trim());

    // Nothing to convert yet → only remember the selection.
    if (!source) return;

    // Auto Detect → show the original spoken-language text.
    if (newLang === 'auto') {
      setDisplayedTranscript(source);
      return;
    }
    ...
    try {
      ...
      const detail = err instanceof Error ? err.message : '';
      setError(detail || 'Failed to translate the transcript.');
    } finally {
      setIsTranslating(false);
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-500">
                ...
                <select
                  ...
                  title="Transcript output language. Auto Detect keeps the original spoken language; selecting a language translates the transcript into it."
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-5 custom-scrollbar bg-white relative">
            ...
            {isGenerating && (
              <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-20 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-slate-600">Creating the clinical report from the edited transcript via OpenAI.</p>
              </div>
            )}
          </div>
        </div>
