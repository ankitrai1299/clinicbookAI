...
// Trust + FAQ + final CTA — AnvayaBook.ai theme. The original used full-dark panels;
// here trust sits on slate-50 with sky/teal accents, and the closing CTA uses the
// platform's slate-900 band (the same dark note the AnvayaBook.ai hero mock uses).
...
const FAQS = [
  ...
  },
  {
    ...
    a: 'Always. AnvayaScribe writes an accurate first draft; you review, edit and only then save, print or send it. Nothing is filed without you.',
  },
  {
  ...
  {
    q: 'Does it share patients with AnvayaBook.ai?',
    a: 'Yes — that is the point. AnvayaScribe and AnvayaBook.ai are two interfaces of one platform, so a patient booked on WhatsApp is the same patient you consult, with one shared timeline.',
  },
  {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <FadeIn>
          ...
          <p className="text-slate-300 mt-5 text-lg max-w-xl mx-auto leading-relaxed">
            Join clinics using AnvayaScribe to remove the documentation burden — and give every visit back to the
            patient.
          </p>
          <button
            onClick={onOpen}
          ...
          >
            {isLoggedIn ? 'Open AnvayaScribe' : 'Sign in to start'}
