...
export default function MobileHome({
  ...
  const stats = React.useMemo(() => {
    ...
    return [
      { id: 'today' as const, icon: <CalendarDays size={18} />, bg: 'bg-[#EEEFFE]', tint: 'text-[#5B5CEB]', label: 'Consultations', value: cur(all), trend: trendOf(cur(all), before(all)) },
      { id: 'drafts' as const, icon: <FileEdit size={18} />, bg: 'bg-[#FEF8EB]', tint: 'text-[#D97706]', label: 'Draft reports', value: cur(draft), trend: trendOf(cur(draft), before(draft)) },
      { id: 'completed' as const, icon: <CheckCircle2 size={18} />, bg: 'bg-[#ECFAF1]', tint: 'text-[#16A34A]', label: 'Completed', value: cur(done), trend: trendOf(cur(done), before(done)) },
      { id: 'follow-ups' as const, icon: <BellRing size={18} />, bg: 'bg-[#EEEFFE]', tint: 'text-[#7C3AED]', label: 'Follow-ups due', value: cur(followUp), trend: trendOf(cur(followUp), before(followUp)) }
    ];
    ...
    <div className="p-5 pb-8">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1 pr-3 min-w-0">
          ...
          <p className="text-slate-500 mt-3 text-[14px] font-medium">{greeting(now)}</p>
      ...
      {onQuickRx && (
        ...
        >
          <Zap size={16} className="text-amber-500" /> Quick prescription
        </button>
      )}
      ...
      <div className="mt-7">
        ...
        {recent.length === 0 ? (
          <EmptyState
            icon={<Mic size={26} />}
            title={consultations.length ? 'Nothing matches' : 'No consultations yet'}
            hint={consultations.length ? 'Try a different name or complaint.' : 'Tap the card above to record your first one.'}
          />
        ) : (
          <div className="space-y-2.5">
            {recent.map((con) => {
