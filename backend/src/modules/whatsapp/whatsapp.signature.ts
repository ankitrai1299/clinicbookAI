...
//
// If WHATSAPP_APP_SECRET is not configured yet, we log a loud warning and allow
// the request through — so local/pre-launch testing still works — but you MUST
// set the secret before exposing the webhook publicly.
let warned = false;
...
export const verifyWhatsAppSignature = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!secret) {
    if (!warned) {
      console.warn('[WhatsApp] WHATSAPP_APP_SECRET not set — inbound webhook signatures are NOT verified. Set it before going live.');
      warned = true;
    }
    return next();
  }
