...
// Inline clinic mark (self-contained SVG — no external asset needed for print).
const LOGO_SVG =
  `<svg width="34" height="34" viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg">` +
  `<rect width="44" height="44" rx="11" fill="#3D5AFE"/>` +
  `<path d="M9 24h6l3-8 4 14 4-20 3 14h6" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/>` +
  `</svg>`;

function letterheadHtml(meta: ReportMeta): string {
  const clinic = escapeHtml(meta.clinicName || 'NovaScribe AI Clinic');
  const docLine = [meta.doctorQualification, meta.doctorRegNo ? `Reg. No. ${meta.doctorRegNo}` : '']
    .map((x) => escapeHtml(x || '')).filter(Boolean).join(' • ');
  const rightRows = [
    meta.consultationId ? `<div><span>Consultation ID</span><b>${escapeHtml(meta.consultationId)}</b></div>` : '',
    (meta.dateTime || meta.date) ? `<div><span>Date</span><b>${escapeHtml(meta.dateTime || meta.date || '')}</b></div>` : '',
    meta.patientName ? `<div><span>Patient</span><b>${escapeHtml(meta.patientName)}</b></div>` : '',
    meta.patientMeta ? `<div><span>&nbsp;</span><b>${escapeHtml(meta.patientMeta)}</b></div>` : '',
  ].filter(Boolean).join('');
  return `<div class="letterhead">
    <div class="lh-left">
      ...
      <div>
        ...
        <div class="lh-doc">${escapeHtml(meta.doctorName || 'Attending Physician')}</div>
        ${docLine ? `<div class="lh-docsub">${docLine}</div>` : ''}
      </div>
    </div>
    <div class="lh-right">${rightRows}</div>
  </div>`;
}
...
function prevSummaryHtml(meta: ReportMeta): string {
  ...
  const row = (label: string, items: string[]) =>
    items.filter(Boolean).length
      ? `<tr><th>${escapeHtml(label)}</th><td>${items.filter(Boolean).map(escapeHtml).join('; ')}</td></tr>`
      : '';
  const rows = [
  ...
  return `<section class="prev">
    <h2>Previous Consultation Summary${p.date ? ` — ${escapeHtml(p.date)}` : ''}</h2>
...
function soapHtml(meta: ReportMeta): string {
  ...
  const block = (label: string, letter: string, text: string) =>
    text.trim()
      ? `<div class="soap-row"><div class="soap-tag">${letter}</div><div class="soap-body"><div class="soap-label">${escapeHtml(label)}</div><div class="soap-text">${escapeHtml(text).replace(/\n/g, '<br/>')}</div></div></div>`
      : '';
  const body = [
  ...
  if (!body) return '';
  return `<section class="soap-section"><h2>SOAP Summary</h2>${body}</section>`;
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .letterhead { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2.5px solid #3D5AFE; padding-bottom: 10px; margin-bottom: 6px; }
  .lh-left { display: flex; gap: 10px; align-items: center; }
  .lh-logo { line-height: 0; }
  .lh-clinic { font-size: 16px; font-weight: 800; color: #0f172a; letter-spacing: 0.2px; }
  .lh-doc { font-size: 12.5px; font-weight: 700; color: #3D5AFE; margin-top: 1px; }
  .lh-docsub { font-size: 10px; color: #64748b; margin-top: 1px; }
  .lh-right { font-size: 10px; text-align: right; }
  .lh-right div { margin-bottom: 2px; }
  .lh-right span { color: #94a3b8; margin-right: 6px; text-transform: uppercase; letter-spacing: 0.3px; font-size: 9px; }
  .lh-right b { color: #1e293b; }
  .title-row { text-align: center; margin: 8px 0 14px; }
  .title-row h1 { font-size: 15px; margin: 0; letter-spacing: 1px; text-transform: uppercase; color: #0f172a; }
  section { margin-bottom: 12px; page-break-inside: avoid; }
  h2 { font-size: 12px; font-weight: 700; color: #3D5AFE; border-bottom: 1.5px solid #cbd5e1; padding-bottom: 3px; margin: 0 0 7px; text-transform: uppercase; letter-spacing: 0.3px; }
  ...
  thead th { background: #eef1ff; font-weight: 700; letter-spacing: 0.2px; color: #1E32B0; }
  table.kv { width: 100%; }
  table.kv th { background: #f8fafc; width: 170px; white-space: nowrap; }
  .group { margin-bottom: 6px; }
  .group-name { font-weight: 700; color: #334155; font-size: 11.5px; margin-bottom: 2px; }
  .prev { background: #f8fafc; border: 1px solid #e2e8f0; border-left: 4px solid #F59E0B; border-radius: 8px; padding: 8px 10px; }
  .prev h2 { color: #B45309; border: none; margin-bottom: 5px; }
  .prev table.kv th { background: transparent; color: #92400e; width: 150px; border-color: #ecd9b5; }
  .prev table.kv td { border-color: #ecd9b5; }
  .soap-section h2 { color: #5A50E8; }
  .soap-row { display: flex; gap: 8px; margin-bottom: 6px; page-break-inside: avoid; }
  .soap-tag { flex: 0 0 22px; height: 22px; border-radius: 6px; background: #6C63FF; color: #fff; font-weight: 800; font-size: 12px; text-align: center; line-height: 22px; }
  .soap-label { font-weight: 700; color: #334155; font-size: 10px; text-transform: uppercase; letter-spacing: 0.3px; }
  .soap-text { white-space: normal; }
  .signature { margin-top: 30px; page-break-inside: avoid; display: flex; justify-content: flex-end; }
  .signature .box { width: 240px; text-align: center; padding-top: 5px; font-size: 11px; color: #334155; }
  .sig-img { max-height: 54px; max-width: 200px; object-fit: contain; margin-bottom: 2px; }
  .signature .line { border-top: 1px solid #334155; padding-top: 5px; }
  .signature .name { font-weight: 700; color: #0f172a; }
  .footer { margin-top: 14px; text-align: center; color: #94a3b8; font-size: 9px; border-top: 1px solid #e2e8f0; padding-top: 6px; }
</style>
</head>
<body>
  ${letterheadHtml(meta)}
  <div class="title-row"><h1>Clinical Consultation Report</h1></div>
  ...
  <div class="footer">Generated by NovaScribe AI • This is a computer-generated clinical document.</div>
</body>
</html>`;
}
