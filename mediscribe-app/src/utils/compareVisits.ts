// ─────────────────────────────────────────────────────────────────────────────
// Compare Previous Visit — a pure, read-only diff of the CURRENT consultation's
// report against the patient's immediately previous visit. Ported from the
// NovaScribe web app so the mobile "Compare Previous Visit" panel behaves
// identically. Creates/duplicates nothing and touches no API/DB/report/session
// logic — it only reads two already-loaded ReportData objects.
// ─────────────────────────────────────────────────────────────────────────────
import { ReportData, Vitals, Consultation } from '../types';
import { VITALS_FIELDS, normalizeReport, formatConsultDate, ReportMeta, L } from './report';

export interface SymptomComparison {
  resolved: string[];   // present last visit, gone now
  added: string[];      // new this visit
  continuing: string[]; // present in both visits
}

export interface VitalChange {
  label: string;
  previous: string;
  current: string;
  direction: 'up' | 'down' | 'same' | null; // numeric trend (first number in each)
}

export interface MedicineChange {
  started: string[];   // started this visit
  stopped: string[];   // on the previous plan, not the current one
  continued: string[]; // continued across both (with a note when regimen changed)
}

export interface TestComparison {
  added: string[];
  removed: string[];
  continuing: string[];
}

export interface VisitComparison {
  symptoms: SymptomComparison;
  vitals: VitalChange[];
  medicines: MedicineChange;
  tests: TestComparison;
  progress: { label: 'Improving' | 'Needs attention' | 'Mixed' | 'Stable'; summary: string } | null;
  hasAny: boolean;
}

const clean = (v: unknown): string => (typeof v === 'string' ? v.trim() : '');
const norm = (v: string): string => v.trim().toLowerCase();

// First number in a free-text vital (e.g. "120/80 mmHg" → 120, "98.6 F" → 98.6).
function firstNumber(value: string): number | null {
  const m = value.match(/-?\d+(\.\d+)?/);
  return m ? Number(m[0]) : null;
}

// Symptom names: prefer structured complaint rows, else the flat chiefComplaint
// projection (complaint text before its " - " suffix). The separator regex
// matches both the current hyphen and the em dash used by older saved reports.
function symptomNames(report: ReportData): string[] {
  const rows = Array.isArray(report.chiefComplaints) ? report.chiefComplaints : [];
  const fromRows = rows.map((r) => clean(r?.complaint)).filter(Boolean);
  if (fromRows.length) return fromRows;
  const flat = Array.isArray(report.chiefComplaint) ? report.chiefComplaint : [];
  return flat.map((s) => clean(s).split(/ [—-] /)[0].trim()).filter(Boolean);
}

// Flattened, de-duplicated diagnostic test / order findings for a report.
function testFindings(report: ReportData): string[] {
  const groups = Array.isArray(report.ordersDiagnostics) ? report.ordersDiagnostics : [];
  const out: string[] = [];
  const seen = new Set<string>();
  for (const g of groups) {
    for (const f of g?.findings || []) {
      const text = clean(f);
      if (text && !seen.has(norm(text))) {
        seen.add(norm(text));
        out.push(text);
      }
    }
  }
  return out;
}

// Diff two string lists case-insensitively, preserving original casing.
function diffLists(prev: string[], curr: string[]) {
  const prevMap = new Map(prev.map((s) => [norm(s), s]));
  const currMap = new Map(curr.map((s) => [norm(s), s]));
  const added: string[] = [];
  const removed: string[] = [];
  const both: string[] = [];
  for (const [k, v] of currMap) (prevMap.has(k) ? both : added).push(v);
  for (const [k, v] of prevMap) if (!currMap.has(k)) removed.push(v);
  return { added, removed, both };
}

/** Whether a report carries any clinical content worth comparing against. */
export function reportHasClinicalContent(report: ReportData): boolean {
  if (symptomNames(report).length) return true;
  if (Array.isArray(report.prescribedMedications) && report.prescribedMedications.some((m) => clean(m?.medicine))) return true;
  if (testFindings(report).length) return true;
  const v = report.clinicalMeasurements || ({} as Vitals);
  if (VITALS_FIELDS.some((f) => clean(v[f.key]))) return true;
  return false;
}

function compareSymptoms(prev: ReportData, curr: ReportData): SymptomComparison {
  const { added, removed, both } = diffLists(symptomNames(prev), symptomNames(curr));
  return { resolved: removed, added, continuing: both };
}

function compareVitals(prev: ReportData, curr: ReportData): VitalChange[] {
  const pv = prev.clinicalMeasurements || ({} as Vitals);
  const cv = curr.clinicalMeasurements || ({} as Vitals);
  const out: VitalChange[] = [];
  for (const f of VITALS_FIELDS) {
    const previous = clean(pv[f.key]);
    const current = clean(cv[f.key]);
    if (!previous || !current) continue; // only compare fields recorded at BOTH visits
    const a = firstNumber(previous);
    const b = firstNumber(current);
    let direction: VitalChange['direction'] = null;
    if (a !== null && b !== null) direction = b > a ? 'up' : b < a ? 'down' : 'same';
    out.push({ label: f.label, previous, current, direction });
  }
  return out;
}

const medName = (m: any): string => clean(m?.medicine);
const medRegimen = (m: any): string =>
  [clean(m?.dose) || clean(m?.dosage), clean(m?.frequency), clean(m?.duration)].filter(Boolean).join(' • ');

function compareMedicines(prev: ReportData, curr: ReportData): MedicineChange {
  const prevRows = (Array.isArray(prev.prescribedMedications) ? prev.prescribedMedications : []).filter((m) => medName(m));
  const currRows = (Array.isArray(curr.prescribedMedications) ? curr.prescribedMedications : []).filter((m) => medName(m));
  const prevMap = new Map(prevRows.map((m) => [norm(medName(m)), m]));
  const currMap = new Map(currRows.map((m) => [norm(medName(m)), m]));

  const started: string[] = [];
  const stopped: string[] = [];
  const continued: string[] = [];

  for (const [k, m] of currMap) {
    if (!prevMap.has(k)) {
      started.push(medName(m));
    } else {
      const before = medRegimen(prevMap.get(k));
      const after = medRegimen(m);
      continued.push(before && after && before !== after ? `${medName(m)} (${before} → ${after})` : medName(m));
    }
  }
  for (const [k, m] of prevMap) if (!currMap.has(k)) stopped.push(medName(m));

  return { started, stopped, continued };
}

function compareTests(prev: ReportData, curr: ReportData): TestComparison {
  const { added, removed, both } = diffLists(testFindings(prev), testFindings(curr));
  return { added, removed, continuing: both };
}

// A gentle, non-prescriptive overall-progress read from symptom resolution and
// pain-score trend. Never claims more than the data supports.
function buildProgress(symptoms: SymptomComparison, prev: ReportData, curr: ReportData, hasAny: boolean): VisitComparison['progress'] {
  if (!hasAny) return null;

  const painPrev = firstNumber(clean(prev.clinicalMeasurements?.painScore || ''));
  const painCurr = firstNumber(clean(curr.clinicalMeasurements?.painScore || ''));
  const painDown = painPrev !== null && painCurr !== null && painCurr < painPrev;
  const painUp = painPrev !== null && painCurr !== null && painCurr > painPrev;

  const improvements = symptoms.resolved.length + (painDown ? 1 : 0);
  const concerns = symptoms.added.length + (painUp ? 1 : 0);

  let label: NonNullable<VisitComparison['progress']>['label'];
  if (improvements > 0 && concerns === 0) label = 'Improving';
  else if (concerns > 0 && improvements === 0) label = 'Needs attention';
  else if (improvements > 0 && concerns > 0) label = 'Mixed';
  else label = 'Stable';

  // Localized like every other label on the document: the summary is printed in
  // the PDF, so it cannot stay English while the report around it is Hindi.
  const count = (n: number, one: string, many: string) => `${n} ${L(n === 1 ? one : many)}`;

  const parts: string[] = [];
  if (symptoms.resolved.length) parts.push(count(symptoms.resolved.length, 'symptom resolved', 'symptoms resolved'));
  if (symptoms.added.length) parts.push(count(symptoms.added.length, 'new symptom', 'new symptoms'));
  if (symptoms.continuing.length) parts.push(count(symptoms.continuing.length, 'ongoing', 'ongoing'));
  if (painDown) parts.push(L('pain score lower'));
  if (painUp) parts.push(L('pain score higher'));

  // L('.') is the sentence terminator, which is a danda (।) in Hindi.
  const summary = parts.length
    ? `${L('Since the previous visit:')} ${parts.join(', ')}${L('.')}`
    : L('Findings are largely unchanged since the previous visit.');

  return { label, summary };
}

export function buildVisitComparison(prev: ReportData, curr: ReportData): VisitComparison {
  const symptoms = compareSymptoms(prev, curr);
  const vitals = compareVitals(prev, curr);
  const medicines = compareMedicines(prev, curr);
  const tests = compareTests(prev, curr);

  const hasAny =
    symptoms.resolved.length > 0 || symptoms.added.length > 0 || symptoms.continuing.length > 0 ||
    vitals.length > 0 ||
    medicines.started.length > 0 || medicines.stopped.length > 0 || medicines.continued.length > 0 ||
    tests.added.length > 0 || tests.removed.length > 0 || tests.continuing.length > 0;

  const progress = buildProgress(symptoms, prev, curr, hasAny);
  return { symptoms, vitals, medicines, tests, progress, hasAny };
}

// ── Previous-visit selection (mirrors the web scoping) ───────
const sessionTime = (c: Consultation): number => {
  const raw = c?.updatedAt || c?.createdAt || c?.date;
  const parsed = raw ? Date.parse(raw) : NaN;
  return Number.isNaN(parsed) ? 0 : parsed;
};

/**
 * The immediately previous visit for this patient: the most recent OTHER session
 * (same patient) at or before the current one that carries clinical content.
 */
/**
 * The `comparison` block for a document's ReportMeta — the "Since Last Visit"
 * panel the PDF/print path renders.
 *
 * Returns undefined when there is no earlier visit or nothing comparable
 * between the two, so the panel is absent rather than printed empty. Every
 * export path (report screen, consultation screen, completed list) builds it
 * through this one function, which is what keeps their PDFs identical.
 */
export function buildComparisonMeta(
  current: Consultation,
  all: Consultation[],
  currentReport: ReportData,
): ReportMeta['comparison'] {
  const previous = findPreviousVisit(current, all);
  if (!previous?.report) return undefined;
  const cmp = buildVisitComparison(normalizeReport(previous.report), currentReport);
  if (!cmp.hasAny) return undefined;
  return { ...cmp, previousDate: formatConsultDate(previous) || previous.date };
}

export function findPreviousVisit(current: Consultation, all: Consultation[]): Consultation | null {
  const key = current.patientId || current.patientName;
  const t = sessionTime(current);
  return (
    all
      .filter((c) => c.id !== current.id && (c.patientId || c.patientName) === key)
      .filter((c) => c.report && reportHasClinicalContent(normalizeReport(c.report)))
      .sort((a, b) => sessionTime(b) - sessionTime(a))
      .find((c) => sessionTime(c) <= t) || null
  );
}
