...
const inputCls =
  'bg-surface border border-slate-200 rounded-md px-2.5 py-2 text-sm text-slate-900';
...
export default function ReportEditor({ report, onChange }: Props) {
          ...
          {cols.map((c) => {
            ...
            return (
              <View key={c.key} className="flex-row">
                <Text className="text-[11px] font-semibold text-slate-500 w-24">{L(c.label)}</Text>
  ...
  return (
    <View className="gap-6">
        ...
        <View key={section.key as string}>
          <View className="flex-row items-center justify-between gap-2 mb-2 border-b border-slate-100 pb-1">
            ...
            >
              ...
              >
                {section.editable ? L('Editable') : L('Read-only')}
              </Text>
            </View>
          </View>
      ...
      {addable.length > 0 && (
        <View>
          <Text className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">{L('Add to report')}</Text>
          <View className="flex-row flex-wrap gap-2">
            {addable.map((s) => (
              <TouchableOpacity
                key={s.key as string}
                ...
                activeOpacity={0.8}
                className="flex-row items-center gap-1.5 px-3 py-2 rounded-full border border-slate-200 bg-surface"
              >
                <Ionicons name="add" size={15} color={colors.brand} />
                <Text className="text-sm font-medium text-slate-600">{L(s.title)}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
