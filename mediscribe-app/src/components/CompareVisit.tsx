...
export default function CompareVisit({
  ...
  previousDate?: string;
  // Rendered as a bordered panel inside another sheet (the report document)
  // rather than as a standalone card, which is how the PDF prints it.
  flat?: boolean;
}) {
  ...
  const Frame: React.ComponentType<any> = flat ? View : Card;

  return (
    <Frame
      className={flat ? 'overflow-hidden rounded-xl border border-slate-200' : 'overflow-hidden p-0'}
      {...(flat ? {} : { elevation: 'sm' as const })}
    >
      {/* Header — same title the printed document's comparison panel carries. */}
      <View className="flex-row items-center gap-2 px-4 py-3 border-b border-slate-100 bg-slate-50">
        <Ionicons name="pulse" size={16} color={colors.docAccent} />
        <Text className="text-[10px] font-bold uppercase text-slate-900" style={{ letterSpacing: 0.9 }}>{L('Since Last Visit')}</Text>
        {previous && previousDate ? <Text className="text-xs text-slate-400">{L('vs')} {previousDate}</Text> : null}
      </View>

      <View className="p-4">
        {!previous ? (
          <Text className="text-sm text-slate-500">{L('No previous visit available for comparison.')}</Text>
        ) : !currentHasReport ? (
          <Text className="text-sm text-slate-500">{L('Generate the current report to compare it with the previous visit.')}</Text>
        ) : !cmp?.hasAny ? (
          <Text className="text-sm text-slate-500">{L('No comparable changes between these two visits.')}</Text>
        ) : (
          <View className="gap-4">
            {/* Symptom changes */}
            <CompareSection
              title={L('Symptom Changes')}
              show={!!(cmp.symptoms.added.length || cmp.symptoms.resolved.length || cmp.symptoms.continuing.length)}
            >
              <View className="gap-2">
                <ChangeGroup label={L('New')} items={cmp.symptoms.added} tone="warn" />
                <ChangeGroup label={L('Resolved')} items={cmp.symptoms.resolved} tone="good" />
                <ChangeGroup label={L('Continuing')} items={cmp.symptoms.continuing} tone="neutral" />
              </View>
            </CompareSection>

            {/* Vital changes */}
            <CompareSection title={L('Vital Changes')} show={cmp.vitals.length > 0}>
              <View className="gap-1.5">
                {cmp.vitals.map((v, i) => (
            ...
            {/* Medicine changes */}
            <CompareSection
              title={L('Medicine Changes')}
              show={!!(cmp.medicines.started.length || cmp.medicines.stopped.length || cmp.medicines.continued.length)}
            >
              <View className="gap-2">
                <ChangeGroup label={L('Started')} items={cmp.medicines.started} tone="info" />
                <ChangeGroup label={L('Stopped')} items={cmp.medicines.stopped} tone="warn" />
                <ChangeGroup label={L('Continued')} items={cmp.medicines.continued} tone="neutral" />
              </View>
            </CompareSection>

            {/* Test result changes */}
            <CompareSection
              title={L('Test Result Changes')}
              show={!!(cmp.tests.added.length || cmp.tests.removed.length || cmp.tests.continuing.length)}
            >
              <View className="gap-2">
                <ChangeGroup label={L('New')} items={cmp.tests.added} tone="info" />
                <ChangeGroup label={L('No longer noted')} items={cmp.tests.removed} tone="neutral" />
                <ChangeGroup label={L('Continuing')} items={cmp.tests.continuing} tone="neutral" />
              </View>
            </CompareSection>

            {/* Overall health progress */}
            {cmp.progress ? (
              <CompareSection title={L('Overall Health Progress')} show>
                <View className="flex-row items-start gap-2">
                  <Chip label={L(cmp.progress.label)} tone={progressTone(cmp.progress.label)} />
