...
const normalizeConsultation = (item: Partial<Consultation> = {}): Consultation => ({
  ...
  //
  // This mapper is a whitelist: anything not named here is silently dropped the
  // next time the list is fetched, which makes a saved field look like it
  // "didn't stick". `language`, the terminology audit trail and the follow-up
  // completion stamp were all being lost that way — each is written by the app
  // and stored by the server, so only this step was discarding them.
  transcriptText: item.transcriptText,
  originalTranscript: item.originalTranscript,
  durationSec: item.durationSec,
  language: item.language,
  uncorrectedTranscript: item.uncorrectedTranscript,
  terminologyCorrections: Array.isArray(item.terminologyCorrections)
    ? item.terminologyCorrections
    : undefined,
...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  // Which doctor this cache belongs to. Everything below is keyed on it.
  const { user } = useAuth();
  const doctorId = user?.id ?? null;

  /** Drop every cached record. Called on sign-out and when the doctor changes. */
  ...
  // Fetch only while a doctor is signed in, and re-fetch when the doctor
  // CHANGES — this cache is per-doctor state, not app state.
  //
  // The clear-on-signout is what stops one doctor's patients from being visible
  // to the next person to use the device: the server would never send them, but
  // without this they would still be sitting in memory (and on screen) after a
  // logout, until a reload replaced them.
  useEffect(() => {
    if (!doctorId) {
      clear();
      setLoading(false);
