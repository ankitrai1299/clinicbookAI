  ...
  return (
    <SafeAreaView className="flex-1 bg-canvas" edges={['top']}>
      <View className="px-5 pt-4 pb-3">
        <Text className="text-[26px] font-extrabold text-slate-900" style={{ letterSpacing: -0.6 }}>{tr('sessions.title')}</Text>
        <Text className="text-slate-500 mt-0.5 text-[13px]">
          {consultations.length} recorded consultation{consultations.length === 1 ? '' : 's'}
        </Text>
      </View>
      <View className="px-5 pb-3">
        <SearchBar value={query} onChangeText={setQuery} placeholder={tr('sessions.searchByDate')} />
      </View>
      ...
      >
        {loading && consultations.length === 0 ? (
          ...
          <EmptyState icon="pulse-outline" title={tr('sessions.noSessionsFound')} subtitle={tr('sessions.willAppear')} />
        ) : (
                ...
                <TouchableOpacity key={c.id} onPress={() => router.push(`/consultation/${c.id}`)} activeOpacity={0.75}>
                  <Card className="p-4" elevation="sm">
                    ...
                    <View className="mt-4 pl-1">
                      <TimelineItem
                        title={tr('sessions.stepRecording')}
                        active={isRecording}
                        done={!isRecording}
                        time={dur || undefined}
                        meta={
                      ...
                      />
                      <TimelineItem title={tr('sessions.stepTranscript')} done={hasTranscript} active={!hasTranscript && !isRecording} />
                      <TimelineItem title={tr('sessions.stepReport')} done={hasReport} active={hasTranscript && !hasReport} last />
                    </View>

                    {/* Actions */}
                    <View className="flex-row gap-2.5 mt-1">
                      <ActionPill icon="document-text-outline" label={tr('sessions.transcript')} tone="success" onPress={() => router.push(`/consultation/${c.id}`)} />
                      <ActionPill
                        icon="sparkles-outline"
                        label={tr('sessions.report')}
                        tone="brand"
